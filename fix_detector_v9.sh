...
"""
Tabela Algılama v9 - AKILLI FİLTRE

Gerçek tabela özellikleri:
  - Kırmızı daire kenarlık  (circ > 0.55)
  - Beyaz iç alan (%30+ beyaz) ← ÖNCEKİ: %10 → yanlış pozitif!
  - Siyah rakam (%5-50 siyah)  ← ÖNCEKİ: >%3 → bariyer %80+ geçiyordu!
  - STOP yazısı: aspect > 1.5  (yatay genişlik)
  - Rakam: aspect 0.3 - 1.5    (dikey veya kare)

ÖNCEKİ SORUN: white=0.12, black=0.86 → bu tabela değil bariyer!
  Gerçek tabela: white=0.40+, black=0.10-0.35
"""
...
class VisionDetector(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'=== DETECTOR v9 (SMART FILTER) r>={self.min_r} cd={self.cooldown}s ===')
    ...
    def detect_sign_contour(self, frame):
        ...
        # İç ROI - daha küçük al (sadece beyaz alan + sayı)
        ir = int(r * 0.50)
        ...
        # ═══ BEYAZLIK: Gerçek tabelanın iç kısmı BEYAZ (>%30) ═══
        ...
        # ═══ SİYAH İÇERİK: Rakam %5-50 alan kaplar, %80 KAPLAMAZ ═══
        gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        _, bw = cv2.threshold(gray, 80, 255, cv2.THRESH_BINARY_INV)
        bf = cv2.countNonZero(bw) / (bw.shape[0] * bw.shape[1])
        
        # LOGLAMA (her aday için)
        self.get_logger().info(
            f'ADAY: ({cx},{cy}) r={r} c={circ:.2f} '
            f'white={wr:.0%} black={bf:.0%}'
        )
        
        # ═══ FİLTRE: Gerçek tabela mı yoksa bariyer mi? ═══
        
        # 1. Beyaz yeterli mi? (tabela iç kısmı beyaz olmalı)
        if wr < 0.25:
            self.get_logger().info(f'  ❌ REJECT: white={wr:.0%} < 25% (bariyer/yol)')
            self.reject_count += 1
            return None
        
        # 2. Siyah çok fazla mı? (rakam max %50 kaplar)
        if bf > 0.50:
            self.get_logger().info(f'  ❌ REJECT: black={bf:.0%} > 50% (bariyer)')
            self.reject_count += 1
            return None
        
        # 3. Siyah çok az mı? (hiç rakam yok?)
        if bf < 0.05:
            self.get_logger().info(f'  ❌ REJECT: black={bf:.0%} < 5% (boş)')
            self.reject_count += 1
            return None
        
        # ═══ GEÇTİ! Tabelanın içine bak ═══
        ...
        self.get_logger().warn(
            f'  ✅ SIGN PASS: white={wr:.0%} black={bf:.0%} aspect={aspect:.2f}'
        )
        
        # STOP vs NUMARA
        if aspect > 2.0 and bf > 0.10:
...
echo "  DETECTOR v9 DONE (SMART FILTER)"
